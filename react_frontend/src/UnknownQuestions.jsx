...
export default function UnknownQuestions({ onBack }) {
  ...
  useEffect(() => {
    ...
    // ✓ حذف chatInterval — فقط عند الحاجة (refresh يدوي أو إرسال رسالة)
    const statsInterval = setInterval(loadStats, 60000);
    const facInterval   = setInterval(fetchFacStats, 60000);
    return () => { clearInterval(srvInterval); clearInterval(statsInterval); clearInterval(facInterval); };
  ...
  const fetchChatbotUsers = async () => {
    /**
     * تحديث عدد المستخدمين النشطين فقط.
     * يُستدعى:
     * 1. عند تحميل الصفحة
     * 2. عند الضغط على الـ Refresh يدوياً
     * لا يوجد استدعاء دوري — فقط الرسائل الفعلية تُسجل كنشاط
     */
    try {
